...
function Button({ children, variant = 'primary', href = '#get-started', className = '' }) {
  return (
...
function MiniAvatar({ initials, tone = 'orange' }) {
  return <span className={`mini-avatar avatar-${tone}`}>{initials}</span>
}

function DashboardPanel({ variant = 'hero' }) {
    ...
    <div className={`dashboard-panel ${compact ? `panel-${variant}` : 'panel-hero'}`}>
      ...
      {variant === 'hero' && (
        <>
          ...
          <div className="panel-week"><div className="week-head"><span>This week</span><span>18 – 22 Jun</span></div><div className="week-bars" aria-label="Weekly attendance: present Monday through Thursday, upcoming Friday">{['M', 'T', 'W', 'T', 'F'].map((day, index) => <div className="week-day" key={`${day}-${index}`}><span className={`week-bar ${index === 4 ? 'week-bar-empty' : ''}`} style={{ '--bar-height': `${48 + index * 9}px` }} /><small>{day}</small></div>)}</div></div>
        </>
      )}
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      media.add({ desktop: '(min-width: 800px)', reduceMotion: '(prefers-reduced-motion: reduce)' }, (conditions) => {
        ...
        gsap.utils.toArray('[data-reveal]').forEach((element) => gsap.from(element, { y: 36, autoAlpha: 0, duration: 0.8, ease: 'power3.out', scrollTrigger: { trigger: element, start: 'top 84%', once: true } }))
        gsap.utils.toArray('.workflow-step').forEach((element, index) => gsap.from(element, { y: 18, autoAlpha: 0, duration: 0.55, delay: index * 0.06, ease: 'power2.out', scrollTrigger: { trigger: '.workflow-section', start: 'top 70%', once: true } }))
        ...
        gsap.utils.toArray('.feature-visual').forEach((visual, index) => {
          gsap.to(visual, { yPercent: index % 2 ? 4 : -4, ease: 'none', scrollTrigger: { trigger: visual, start: 'top bottom', end: 'bottom top', scrub: 1.2 } })
        })
        gsap.utils.toArray('.feature-visual .dashboard-panel').forEach((panel) => {
        ...
        const interactive = gsap.utils.toArray('.button, .role-card, .feature-visual, .workflow-node')
